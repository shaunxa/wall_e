"""Wall-E controller with touch toggle, idle head sweep, and moving eye mode.
...
# Idle animation: wait five seconds, left over three seconds, then right over five.
IDLE_START_MS = 5000
IDLE_LEFT_OFFSET = -20
...
# Moving/"excited" eye: Color Sensor's three individual RGB light segments.
EXCITING_COLOR_INTERVAL_MS = 1000
EXCITING_EYE_REFRESH_MS = 100
RGB_SEGMENTS = ((100, 0, 0), (0, 100, 0), (0, 0, 100))
...
def set_idle_eyes(on, mood="awake"):
    """Use the four ultrasonic LEDs for quiet happy/sad idle expressions."""
...
def set_exciting_eyes(color_index):
    """All four ultrasonic LEDs on; rotate one RGB Color Sensor segment."""
    global eyes_are_on, eye_mood
    try:
        distance_eye.lights.on(100)
        color_eye.lights.on(RGB_SEGMENTS[color_index])
        eyes_are_on = True
        eye_mood = "excited"
    except OSError:
        pass
...
def run_head_target(logical_angle, duration_ms, logical_distance):
    motor_angle = HEAD_SIGN * logical_angle * HEAD_HEAD_TEETH // HEAD_MOTOR_TEETH
    speed = max(10, logical_distance * HEAD_HEAD_TEETH * 1000 // (HEAD_MOTOR_TEETH * duration_ms))
    head.run_target(speed, motor_angle, wait=False)
...
def animate_personality(now):
    ...
    if wheels_moving:
        idle_active = False
        idle_phase = 0
        if not exciting_active:
            exciting_active = True
            exciting_color_index = 0
            last_exciting_color_ms = now
            last_exciting_eye_refresh_ms = 0
        elif now - last_exciting_color_ms >= EXCITING_COLOR_INTERVAL_MS:
            exciting_color_index = (exciting_color_index + 1) % len(RGB_SEGMENTS)
            last_exciting_color_ms = now
        if now - last_exciting_eye_refresh_ms >= EXCITING_EYE_REFRESH_MS:
            # ColorSensor.reflection() can temporarily alter its emitter; refresh it.
            set_exciting_eyes(exciting_color_index)
            last_exciting_eye_refresh_ms = now
        return

    exciting_active = False
    if now - last_motion_ms < IDLE_START_MS:
        set_idle_eyes(True, "awake")
        return
    ...
    if not idle_active:
        ...
        run_head_target(clamp(head_home_angle + IDLE_LEFT_OFFSET, -HEAD_LIMIT, HEAD_LIMIT), IDLE_LEFT_TIME_MS, abs(IDLE_LEFT_OFFSET))
    elif now - idle_phase_started_ms >= idle_phase_duration_ms:
        idle_phase = (idle_phase + 1) % 3
        idle_phase_started_ms = now
        if idle_phase == 1:
            idle_phase_duration_ms = IDLE_RIGHT_TIME_MS
            run_head_target(clamp(head_home_angle + IDLE_RIGHT_OFFSET, -HEAD_LIMIT, HEAD_LIMIT), IDLE_RIGHT_TIME_MS, abs(IDLE_RIGHT_OFFSET - IDLE_LEFT_OFFSET))
        elif idle_phase == 2:
    ...
    if blink_until_ms:
        if now >= blink_until_ms:
            set_idle_eyes(True, "happy" if getrandbits(1) else "sad")
            blink_until_ms = 0
...
while True:
    ...
    animate_personality(now)

    if now - last_telemetry_ms >= TELEMETRY_INTERVAL_MS:
        last_telemetry_ms = now
        send("TEL d={} r={} touch={} h={} wheels={}".format(
            distance_eye.distance(), color_eye.reflection(), int(shoulder_pressed),
